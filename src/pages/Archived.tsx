...
import { Mail, Calendar, MessageSquare, ChevronRight, ArrowLeft, Edit2, Save, X, Phone, Archive, CheckSquare, Sparkles, Plus, ArrowUpDown, Clock, CalendarDays, MessageCircle, Mic, Send, ChevronDown, CheckCircle, XCircle, StickyNote, LogOut } from "lucide-react";
...
const Archived = () => {
  ...
  const viewMode = 'archived'; // Fixo para esta página
  ...
  const [sortType, setSortType] = useState<'recent-message' | 'recent-inbound' | 'newest' | 'oldest' | 'no-response'>('recent-inbound');
  ...
  const [produtoFilter, setProdutoFilter] = useState<'all' | 'publicidade' | 'palestra' | 'consultoria' | 'palestra_consultoria'>('all');
  ...
  useEffect(() => {
    ...
    // 'all' mostra todos incluindo perdidos
    ...
    if (pendingResponseFilter) {
      filteredLeads = filteredLeads.filter(lead => {
        ...
        // Pendente quando a última mensagem do lead (inbound) é a mais recente
  ...
  const fetchLeads = async () => {
    try {
      ...
      // Filtrar baseado no modo de visualização - fixo como archived
      ...
      <div className="max-w-6xl mx-auto overflow-x-hidden">
        <div className="mb-8">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h1 className="text-4xl font-bold mb-2">Arquivados</h1>
