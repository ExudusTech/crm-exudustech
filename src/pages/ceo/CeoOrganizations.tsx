    ...
    <div className="p-6 space-y-4 max-w-7xl mx-auto">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
            <Building2 className="h-6 w-6" /> Organizações
          </h1>
      ...
      {loading ? (
        ...
        <div className="rounded-md border">
          <Table>
            ...
            <TableBody>
              {filtered.map((org) => (
                <TableRow key={org.id} className="cursor-pointer hover:bg-muted/50" onClick={() => openEdit(org)}>
                  ...
                  <TableCell className="text-sm text-muted-foreground">{org.segment || "—"}</TableCell>
                  <TableCell><Badge className={statusColors[org.status]}>{ceoStatusLabels[org.status]}</Badge></TableCell>
                  <TableCell>
                    {org.website ? (
                      <a href={org.website} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline" onClick={(e) => e.stopPropagation()}>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>{editing.id ? "Editar Organização" : "Nova Organização"}</DialogTitle>
          </DialogHeader>
